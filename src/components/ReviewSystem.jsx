  ...
  return (
    <div className="review-system min-w-full">
      ...
      <div className="flex justify-center">
        <button onClick={() => setIsModalOpen(true)} className="px-6 py-3 border border-orange-500 text-orange-500 rounded hover:bg-orange-500 hover:text-white transition">
          Give Your Review
        </button>
      </div>
      ...
      {isModalOpen && (
        <div className="modal fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50">
          <div className="modal-content p-10 w-auto border border-neutral-700 rounded-xl backdrop-blur-lg flex flex-col w-80">
            <h2 className='mb-5 text-xl'>Leave a Review</h2>
            {error && <p className="text-red-500">{error}</p>}
            <div>
              <label className="block mb-2">Name:</label>
              <input type="text" value={name} className='p-2 border border-neutral-300 rounded mb-4' onChange={(e) => setName(e.target.value)} />
            </div>
            <div>
              <label className="block mb-2">Comment:</label>
              <textarea value={comment} onChange={(e) => setComment(e.target.value)} className='p-2 border border-neutral-300 rounded mb-4' />
            </div>
            <div className="star-rating mb-4">
              <label className="block mb-2">Rating:</label>
              {[...Array(5)].map((_, index) => (
                <button type="button" key={index} onClick={() => setRating(index + 1)} style={{ color: index < rating ? '#ffd700' : '#ccc' }} className="text-2xl">
                  ★
                </button>
              ))}
            </div>
            <div className="flex justify-between">
              <button onClick={() => setIsModalOpen(false)} className="px-6 py-3 border border-gray-500 text-gray-500 rounded hover:bg-gray-500 hover:text-white transition">Cancel</button>
              <button onClick={handleSubmit} className="px-6 py-3 border border-orange-500 text-orange-500 rounded hover:bg-orange-500 hover:text-white transition">Submit</button>
            </div>
          </div>
        </div>
      )}
